...
def make_out_of_turn_move(player, current_gameboard, allowable_moves, code):
    ...
    #         #####becky##########################################################################################
    #         #add a dict() for including all the property can vbe improved
    #         params_all = agent_helper_functions.identify_improvement_opportunity_all(player, current_gameboard)
    #         ####################################################################################################
    #         if param:
...
def make_post_roll_move(player, current_gameboard, allowable_moves, code, move_actions):
    """
    ...
    if move_actions:
        ...
        if action_choices.improve_property in move_action:  # beef up full color sets to maximize rent potential.
            print player.player_name, ': I am going to improve property ',space_action[0].name
            params = dict()
            params['player'] = player
