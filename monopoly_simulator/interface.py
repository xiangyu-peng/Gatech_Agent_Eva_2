...
class Interface(object):
    ...
    def action_num2vec(self, action):
        actions_vec = [0 for i in range(1+ 22 + 28 + 28)]
        actions_vec[int(action)] = 1
        return actions_vec

    #action space 1+22+28+28 = 79
    def action_space(self, current_board, current_player):
        action_space = []
        site_space = []
        # 1 first denotes the action -> buy or not buy
        action_space.append(buy_property)
        site_space.append(current_board['location_objects'][self.board_state[current_player.current_position]])
    ...
    def vector_to_actions(self, current_board, current_player, actions_vector=actions_vector_default):
        move_actions = []
        action_space, site_space = self.action_space(current_board, current_player)
        print('actions_vector =====> ', actions_vector)
        try:
