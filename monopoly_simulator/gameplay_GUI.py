...
class CreateBoardWindow(Screen):
    ...
    def create_board(self):
        ...
        for i in range(each_side_number_of_props):
            ...
            num_tiles_spanned_by_property = self.game_elem['location_sequence'][i].end_position - \
                                            self.game_elem['location_sequence'][i].start_position
            try:
            ...
            for j in range(num_tiles_spanned_by_property):
                property_button = PropertyButton(text=label,
                                                 ...
                                                 size_hint=(each_block_size, each_block_size))
    ...
    def simulate_game_instance(self, history_log_file=None, np_seed=6):
        ...
        logger.debug('players will play in the following order: ' + '->'.join(
            ...
            if self.start_stop_flag == True:
                ...
                while skip_turn != num_active_players and out_of_turn_count <= 200:
                    ...
                    out_of_turn_player = self.game_elem['players'][
                        out_of_turn_player_index % len(self.game_elem['players'])]
                ...
                if diagnostics.max_cash_balance(
                        self.game_elem) > 300000:  # this is our limit for runaway cash for testing purposes only.
                    ...
                    if list(self.game_elem['players'][i].assets)[k].loc_class == 'real_estate':
                        if list(self.game_elem['players'][i].assets)[k].num_houses > 0:
                            for m in range(list(self.game_elem['players'][i].assets)[k].num_houses):
                                ...
                                self.obj_instr.add(Ellipse(
                                    pos=((pos_x_prop + (m + 1) * 0.01) * Window.width, pos_y_prop * Window.height),
                                    size=(12, 12)))
