...
class MonopolyLossCal(object):
    def __init__(self, params,
        ...
        if pretrain_model:
            self.model = torch.load(pretrain_model)
        else:
    ...
    def test_v2(self, step_idx, seed, config_file=None):
    ...
    def loss_cal(self):
        ...
        loss_return = torch.tensor(0, device=self.device).float()
        while step_idx < self.max_train_steps and self.spend_time < 3:
            ...
            if step_idx % self.PRINT_INTERVAL == 0:
                ...
                print('loss', loss_train / self.PRINT_INTERVAL, prob)
...
if __name__ == '__main__':
    ...
    config_file = '/Hypothetical_simulator/config_online_baseline.ini'
    config_data = ConfigParser()
    config_data.read('/media/becky/GNOME-p3/Hypothetical_simulator/config_online_baseline.ini')
    ...
    for params in param_list:  # run different hyper parameters in sequence
        trainer = MonopolyTrainer(params, device_id, gameboard=None, kg_use=False, logger_use=True, config_file=config_file, test_required=True)
        trainer.train()
