...
class History_Record(object):
    ...
    def save_file(self, save_dict, save_path):
        '''
    ...
    def read_file(self, load_dict, load_path):
        '''
...
class KG_OpenIE_eva(History_Record):
    def __init__(self, gameboard=None,\
        ...
        self.relations = ['priced', 'rented', 'located', 'colored', 'classified', 'away', 'type', 'cost', 'direct', 'mortgaged']
...
class Novelty_Detection_Dice(History_Record):
    def __init__(self, config_file=upper_path+'/A2C_agent/config.ini'):
        #Novelty Detection
        config_data = ConfigParser()
        config_data.read(config_file)
...
if __name__ == '__main__':
    ...
    client = KG_OpenIE_eva(gameboard_initial)
    print(client.annotate('Luxury-Tax is cost at 100'))
