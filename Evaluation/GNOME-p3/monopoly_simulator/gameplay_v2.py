...
###GT - Call the agent#####
import os, sys
sys.path.append('/'.join(os.getcwd().split('/')[:-1]))  # Add the parent directory to the path
...
def simulate_game_instance(game_elements, history_log_file=None, np_seed=124):
    """
    ...
    while num_active_players > 1:
        ...
        # print(game_elements['location_sequence'])
...
#
#     game_elements = set_up_board('/media/becky/Evaluation/GNOME-p3/monopoly_game_schema_v1-2.json',
...
#
#     if player_decision_agents['player_1'].startup(game_elements) == -1 or player_decision_agents['player_2'].startup(
#             game_elements) == -1 or \
...
def play_game_in_tournament(game_seed, inject_novelty_function=None, agent=None):
    ...
    if player_decision_agents['player_1'].startup(game_elements) == -1 or player_decision_agents['player_2'].startup(
        ...
        if player_decision_agents['player_1'].shutdown() == -1 or player_decision_agents['player_2'].shutdown() == -1 or \
                player_decision_agents['player_3'].shutdown() == -1 or \
                player_decision_agents['player_4'].shutdown() == -1:
            logger.error("Error in agent shutdown.")
...
# print(play_game_in_tournament(3))
