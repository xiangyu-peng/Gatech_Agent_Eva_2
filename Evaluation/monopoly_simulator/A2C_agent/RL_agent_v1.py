...
sys.path.append('/media/becky/GNOME-p3/Evaluation/monopoly_simulator')
...
def handle_negative_cash_balance(player, current_gameboard):
    ...
    while (
            player.num_total_houses > 0 or player.num_total_hotels > 0) and count < 3:  # often times, a sale may not succeed due to uniformity requirements. We keep trying till everything is sold,
    ...
    for a in sorted_player_assets_list:
        action_choices.sell_property(player, a,
                                     current_gameboard)  # this could be refined further; we may be able to get away with a mortgage at this point.
        if player.current_cash >= 0:
            return 1  # we're done
    ...
    #     # a.save_bool(1)
    #     a.set_board(current_gameboard)
    #     # a.save_history(self, current_gameboard,
    #     #                save_path='/media/becky/Evaluation/GNOME-p3/monopoly_simulator/loc_history.pickle')
    #
    #     a.get_logging_info_once(current_gameboard)
    #     a.get_logging_info(current_gameboard)  # , current_gameboard['players'].index(player)
    #
    #     #######Add kg here######
    #     kg_class_path = upper_path + '/A2C_agent/log/kg_rel.pickle'
    #     kg = KG_OpenIE(current_gameboard)
    #
    #     if os.path.exists(kg_class_path):
    #         with open(kg_class_path, 'rb') as f:
    #             kg.kg_rel, kg.kg_set, kg.update_num, kg.kg_change, kg.location_record, kg.dice\
    #                 , kg.text_dice_num, kg.new_kg_tuple, kg.kg_rel_diff, kg.dice_novelty = pickle.load(f)
    #
    #     else:
    #         file = open(kg_class_path, 'wb')
    #         file.close()
    #     kg.update_num += 1
    #     kg.set_gameboard(current_gameboard)
    #     kg.build_kg_file(upper_path+'/A2C_agent/log/game_log.txt', level='rel', use_hash=False)
    #     kg.save_file([kg.kg_rel, kg.kg_set, kg.update_num, kg.kg_change, kg.location_record, kg.dice\
    #                 , kg.text_dice_num, kg.new_kg_tuple, kg.kg_rel_diff, kg.dice_novelty], kg_class_path)
    #
    #     a.clear_history()
