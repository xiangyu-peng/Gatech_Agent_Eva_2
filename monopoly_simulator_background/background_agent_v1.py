import monopoly_simulator_background.action_choices
import monopoly_simulator_background.agent_helper_functions # helper functions are internal to the agent and will not be recorded in the function log.
from monopoly_simulator_background import diagnostics
from monopoly_simulator_background import hypothetical_simulator
import logging
...
def make_pre_roll_move(player, current_gameboard, allowable_moves, code):
    ...
    if player.current_cash >= current_gameboard['go_increment']: # if we don't have enough money, best to stay put.
        ...
        if monopoly_simulator_background.action_choices.use_get_out_of_jail_card in allowable_moves:
            ...
            player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.use_get_out_of_jail_card
            ...
            player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.pay_jail_fine
    ...
    if monopoly_simulator_background.action_choices.skip_turn in allowable_moves:
        ...
        player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.skip_turn
        ...
        # player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.concluded_actions
...
def make_out_of_turn_move(player, current_gameboard, allowable_moves, code):
    ...
    if monopoly_simulator_background.action_choices.accept_trade_offer in allowable_moves:
    ...
    elif monopoly_simulator_background.action_choices.accept_trade_offer in allowable_moves:
        param = dict()
        param['player'] = player
        param['current_gameboard'] = current_gameboard
        ...
        if list(player.outstanding_trade_offer['property_set_offered'])[0].is_mortgaged or player.outstanding_trade_offer['cash_wanted']>player.current_cash:
            ...
            player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.accept_sell_property_offer
            return (monopoly_simulator_background.action_choices.accept_trade_offer, param)
        elif monopoly_simulator_background.agent_helper_functions.will_property_complete_set(player, list(player.outstanding_trade_offer['property_set_offered'])[0],current_gameboard):
            ...
            if player.current_cash - player.outstanding_trade_offer['cash_wanted'] >= current_gameboard['go_increment']/2: # risky, but worth it
                ...
                player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.accept_trade_offer
                return (monopoly_simulator_background.action_choices.accept_trade_offer, param)

    if monopoly_simulator_background.action_choices.accept_sell_property_offer in allowable_moves:
        ...
        if player.outstanding_property_offer['asset'].is_mortgaged or player.outstanding_property_offer['price']>player.current_cash:
            ...
            logger.debug(player.player_name+ ': I am accepting the offer to buy '+player.outstanding_property_offer['asset'].name+' since I can afford' \
                                                    'it and it is being offered at or below market rate.')
            player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.accept_sell_property_offer
            return (monopoly_simulator_background.action_choices.accept_sell_property_offer, param)
        elif monopoly_simulator_background.agent_helper_functions.will_property_complete_set(player, player.outstanding_property_offer['asset'],current_gameboard):
            # 2. less affordable, but we stand to gain by monopoly
    ...
    if player.status != 'current_move': # these actions are considered only if it's NOT our turn to roll the dice.
        if monopoly_simulator_background.action_choices.improve_property in allowable_moves: # beef up full color sets to maximize rent potential.
            param = monopoly_simulator_background.agent_helper_functions.identify_improvement_opportunity(player, current_gameboard)
            if param:
                if player.agent._agent_memory['previous_action'] == monopoly_simulator_background.action_choices.improve_property and code == -1:
                    ...
                    player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.improve_property
                    return (monopoly_simulator_background.action_choices.improve_property, param)
        ...
        for m in player_mortgaged_assets_list:
            if player.current_cash-(m.mortgage*1.1) >= current_gameboard['go_increment'] and monopoly_simulator_background.action_choices.free_mortgage in allowable_moves:
                ...
                player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.free_mortgage
                return (monopoly_simulator_background.action_choices.free_mortgage, param)


    else: # it is our current move. We should raise money if necessary, especially if we're low on cash.
        if player.current_cash < current_gameboard['go_increment'] and monopoly_simulator_background.action_choices.make_trade_offer in allowable_moves:
            ...
            potential_offer_list = monopoly_simulator_background.agent_helper_functions.identify_property_trade_offer_to_player(player, current_gameboard)
            potential_request_list = None
            param = monopoly_simulator_background.agent_helper_functions.curate_trade_offer(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag=1)
            if param and player.agent._agent_memory['previous_action'] != monopoly_simulator_background.action_choices.make_trade_offer: # we only make one offer per turn. Otherwise we'd
                # be stuck in a loop
                ...
                player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.make_trade_offer
                return (monopoly_simulator_background.action_choices.make_trade_offer, param)
        elif monopoly_simulator_background.action_choices.make_trade_offer in allowable_moves: # let's try to see if we can sell property to other players at an insane premium
...
def make_post_roll_move(player, current_gameboard, allowable_moves, code):
    ...
    if monopoly_simulator_background.action_choices.buy_property in allowable_moves:
        if code == -1:
        ...
        if make_buy_property_decision(player, current_gameboard, params['asset']):
            ...
            player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.buy_property
            return (monopoly_simulator_background.action_choices.buy_property, params)
        else:
            # make_buy returned false, but is there still a chance?
            if monopoly_simulator_background.agent_helper_functions.will_property_complete_set(player,current_location,current_gameboard):
                # if we can raise enough money, then the 'next' time around we'll succeed in buying
                to_mortgage = monopoly_simulator_background.agent_helper_functions.identify_potential_mortgage(player,current_location.price,True)
                if to_mortgage:
                    params['asset'] = to_mortgage
                    logger.debug(player.player_name+ ': I am attempting to mortgage property '+ params['asset'].name)
                    player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.mortgage_property
                    return (monopoly_simulator_background.action_choices.mortgage_property, params)

                else: # last chance.
                    to_sell = monopoly_simulator_background.agent_helper_functions.identify_potential_sale(player, current_location.price,True)
                    if to_sell:
                        params['asset'] = to_sell
                        logger.debug(player.player_name+ ': I am attempting to sell property '+ params['asset'].name+' to the bank')
                        player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.sell_property
                        return (monopoly_simulator_background.action_choices.sell_property, params)

    if monopoly_simulator_background.action_choices.concluded_actions in allowable_moves:
        # player.agent._agent_memory['previous_action'] = monopoly_simulator_background.action_choices.concluded_actions
        return (monopoly_simulator_background.action_choices.concluded_actions, dict())
...
def make_buy_property_decision(player, current_gameboard, asset):
    ...
    if player.current_cash - asset.price >= current_gameboard['go_increment']:  # case 1: can we afford it?
        ...
        decision = True
            ...
            monopoly_simulator_background.agent_helper_functions.will_property_complete_set(player,asset,current_gameboard):
        logger.debug(player.player_name+ ': I will attempt to buy '+ asset.name+ ' from the bank.')
        decision = True
...
def make_bid(player, current_gameboard, asset, current_bid):
    ...
    elif current_bid < player.current_cash and monopoly_simulator_background.agent_helper_functions.will_property_complete_set(player,asset,current_gameboard):
...
def handle_negative_cash_balance(player, current_gameboard):
    ...
    if mortgage_potentials and max_sum+player.current_cash >= 0: # if the second condition is not met, no point in mortgaging
        ...
        for p in sorted_potentials:
            if player.current_cash >= 0:
                return 1 # we're done
            monopoly_simulator_background.action_choices.mortgage_property(player, p[0], current_gameboard)
    ...
    if sale_potentials: # if the second condition is not met, no point in mortgaging
        ...
        for p in sorted_potentials:
            if player.current_cash >= 0:
                return 1 # we're done
            monopoly_simulator_background.action_choices.sell_property(player, p[0], current_gameboard)
            ...
            for prop in sorted_player_assets_list:
                if prop!=p[0] and prop.color==p[0].color and p[0].color in player.full_color_sets_possessed:
                    if prop.num_hotels>0:
                        monopoly_simulator_background.action_choices.sell_house_hotel(player, prop, current_gameboard, False, True)
                        if player.current_cash >= 0:
                            return 1
                    elif prop.num_houses>0:
                        while prop.num_houses>0:
                            monopoly_simulator_background.action_choices.sell_house_hotel(player, prop, current_gameboard, True, False)
                            if player.current_cash >= 0:
                                return 1
                    else:
                        continue
    ...
    while (player.num_total_houses > 0 or player.num_total_hotels > 0) and count <3: # often times, a sale may not succeed due to uniformity requirements. We keep trying till everything is sold,
        ...
        for a in sorted_assets_list:
            if a.num_houses > 0:
                monopoly_simulator_background.action_choices.sell_house_hotel(player, a, current_gameboard,True, False)
                if player.current_cash >= 0:
                    return 1 # we're done
            elif a.num_hotels > 0:
                monopoly_simulator_background.action_choices.sell_house_hotel(player, a, current_gameboard, False, True)
                if player.current_cash >= 0:
                    return 1  # we're done
    ...
    for a in sorted_player_assets_list:
        monopoly_simulator_background.action_choices.sell_property(player, a, current_gameboard) # this could be refined further; we may be able to get away with a mortgage at this point.
        if player.current_cash >= 0:
            return 1  # we're done
