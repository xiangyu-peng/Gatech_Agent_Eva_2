...
"""
import sys
upper_path_env = '/media/becky/GNOME-p3'
sys.path.append(upper_path_env)

from monopoly_simulator_background import location
from monopoly_simulator_background.dice import Dice
from monopoly_simulator_background.bank import Bank
from monopoly_simulator_background.card_utility_actions import * # functions from this module will be used in reflections in initialize_board,
                                    # and excluding this import will lead to run-time errors
import sys
from monopoly_simulator_background.player import Player
# from player_becky_easy import Player
from monopoly_simulator_background import card
import copy
import logging
...
def _initialize_dies(game_elements, game_schema):
    if len(game_schema['die']['die_state']) != game_schema['die']['die_count']:
        looger.debug('dice count is unequal to number of specified dice state-vectors...')
        logger.error("Exception")
...
def _initialize_cards(game_elements, game_schema):
    ...
    for specific_card in game_schema['cards']['community_chest']['card_states']:
        ...
        if specific_card['card_type'] == 'movement':
            ...
            looger.debug('community chest card type is not recognized: ', specific_card['card_type'])
            logger.error("Exception")
    ...
    if len(community_chest_cards) != game_schema['cards']['community_chest']['card_count']:
        looger.debug('community chest card count and number of items in community chest card set are inconsistent')
    ...
    for specific_card in game_schema['cards']['chance']['card_states']:
        ...
        if specific_card['card_type'] == 'movement':
            ...
            looger.debug('chance card type is not recognized: ', specific_card['card_type'])
            logger.error("Exception")

        chance_card_objects[card_obj.name] = card_obj

    if len(chance_cards) != game_schema['cards']['chance']['card_count']:
        looger.debug('chance card count and number of items in chance card set are inconsistent')
