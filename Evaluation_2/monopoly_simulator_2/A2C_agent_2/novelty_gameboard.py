...
def detect_card_nevelty(current_gameboard, gameboard_ini):
    ...
    """
    novelty = []
    print(current_gameboard['cards'].keys())
    card_keys = ['chance_cards', 'community_chest_cards']
    for card_key in card_keys:
        if card_type_detect(current_gameboard['cards'][card_key]) != card_type_detect(gameboard_ini['cards'][card_key]):
            # print('different')
            novelty = dict_difference(card_type_detect(gameboard_ini['cards'][card_key]), card_type_detect(current_gameboard['cards'][card_key]))
        # else:
        #     print('same')
    if novelty:
        return novelty
