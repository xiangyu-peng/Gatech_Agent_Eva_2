...
sys.path.append('/media/becky/GNOME-p3/Evaluation_2/monopoly_simulator')
from monopoly_simulator import action_choices
import A2C_agent.agent_helper_functions_agent as agent_helper_functions # helper functions are internal to the agent and will not be recorded in the function log.
...
def make_out_of_turn_move(serial_dict_to_client, memory):
    ...
    #     logger.debug(player_name + ': Should I accept the offer by ' + player['outstanding_property_offer'][
    ...
    if player['status'] != 'current_move': # these actions are considered only if it's NOT our turn to roll the dice.
        ...
        for m in player_mortgaged_assets_list:
            ...
            if player['current_cash'] - (m['mortgage']*(1+ 0.1)) >= 150 and "free_mortgage" in allowable_moves:
                # free mortgages till we can afford it. the second condition should not be necessary but just in case.
                param = dict()
                param['player'] = 'player_1'
                param['asset'] = m['name']
        ...
        if player['current_cash'] < 150 and "make_trade_offer" in allowable_moves:
            # in this case, the trade offer is a duplication of make_sell_property_offer since the player is in urgent need of cash and
            #cannot strategize a trade
...
def make_post_roll_move(return_to_server_dict):
    """
    ...
    # a.save_bool(0)
    a.get_logging_info_once(current_gameboard)

    s = a.board_to_state(current_gameboard)
    s = s.reshape(1, -1)

#############
    device = torch.device('cpu')
    model_path = upper_path + '/A2C_agent/weights/push_buy_tf_ne_v3_45.pkl'
    ...
    action = action[0]
    actions_vector = a.action_num2vec(action)
    move_actions = a.vector_to_actions(current_gameboard, player, actions_vector)
    #####################
    ...
    if action_choices.buy_property in move_action:
        if action_choices.buy_property in allowable_moves:
            ...
            if code == -1:
                logger.debug(
                    'player_1' + ': I did not succeed the last time in buying this property. Concluding actions...')
                return (action_choices.concluded_actions, dict())

            logger.debug('player_1' + ': I am attempting to buy property ' + params['asset']['name'])
    ...
    if action_choices.improve_property in move_action:  # beef up full color sets to maximize rent potential.
        if action_choices.improve_property in allowable_moves:
            logger.debug('player_1' + ': I am going to improve property ' + space_action[0]['name'])
            params = dict()
            params['player'] = player
    ...
    if action_choices.free_mortgage in move_action:
        ...
        # free mortgages till we can afford it. the second condition should not be necessary but just in case.
            ...
            params['asset'] = space_action[0]
            params['current_gameboard'] = current_gameboard
            logger.debug('player_1' + ': I am going to free mortgage on ' + space_action[0]['name'])
            player.agent._agent_memory['previous_action'] = action_choices.free_mortgage
    ...
    if action_choices.mortgage_property in move_action:
        if action_choices.mortgage_property in allowable_moves:
            ...
            params['asset'] = space_action[0]
            params['current_gameboard'] = current_gameboard
            logger.debug('player_1' + ': I am attempting to mortgage property ' + space_action[0]['name'])
...
def make_buy_property_decision(serial_dict_to_client):
    ...
    decision = False
    if player['current_cash'] - asset['price'] >= 150:  # case 1: can we afford it?
...
def handle_negative_cash_balance(serial_dict_to_client, memory):
    """
    ...
    for a in sorted_player_assets_list:
        ...
        if a['color'] in player['full_color_sets_possessed']:
            ...
            sale_potentials.append((a, (a['price'] * 0.5)-((1 + 0.1)*a['mortgage']))) #current_gameboard['bank'].property_sell_percentage
        elif a['loc_class'] =='real_estate' and (a['num_houses'] >0 or a['num_hotels'] >0):
            continue
        else:
            sale_potentials.append((a, a['price'] * 0.5)) #* current_gameboard['bank'].property_sell_percentage
    ...
    for a in sorted_player_assets_list:
        a = current_gameboard['locations'][a]
        if a['is_mortgaged']:
            sale_potentials.append((a, (a['price'] * 0.5)-((1+0.1)*a['mortgage'])))
        elif a['loc_class'] =='real_estate' and (a['num_houses']>0 or a['num_hotels']>0):
            continue
        else:
            sale_potentials.append((a,a['price']*0.5))
