...
# python gat_part_hyp_v2.py  --novelty_change_num 20 --novelty_change_begin 1 --novelty_introduce_begin 0 --retrain_type gat_part --exp_name 20_1_h --device_id 2 --pretrain_model /media/becky/GNOME-p3/monopoly_simulator_background/weights0_0_gat_part_seed_1048000.pkl --seed 9
# nohup python gat_part_hyp_v2.py  --novelty_change_num 20 --novelty_change_begin 1 --novelty_introduce_begin 0 --retrain_type baseline --kg_use False --exp_name 20_1_h --device_id 1 --pretrain_model /media/becky/GNOME-p3/monopoly_simulator_background/weights19_1_baseline_seed_9147000.pkl --seed 10
import numpy as np
from model import *
...
class MonopolyTrainer_GAT:
    def __init__(self,
        ...
        if pretrain_model:
            ...
            if 'gat' in self.retrain_type:
                ...
                self.config_model.entity_id_file_path = self.params['entity_id_file_path'] + '/entity_id_30_1_tryenv.json'
        ...
        for (i, name) in enumerate(
            ...
            if i >= self.exp_dict['novelty_num'][1] - 1 and i < self.exp_dict['novelty_num'][1] + self.exp_dict['novelty_num'][0] - 1:
    ...
    def load_adj(self):
        ...
        if os.path.exists(self.adj_path):
            ...
            #     self.adj = self.adj[:, self.adj.shape[0] * i: self.adj.shape[0] * (i + 1)] + \
            #                self.adj[:, self.adj.shape[0] * (i + 1): self.adj.shape[0] * (i + 2)]
    ...
    def set_gameboard(self,
        ...
        with HiddenPrints():
            self.envs.close()
            self.envs = ParallelEnv(self.n_train_processes, self.gameboard, self.kg_use, self.config_file,
                                    self.seed,
                                    self.exp_dict)
    ...
    def train(self):
        ...
        while step_idx < self.max_train_steps and self.spend_time < 300:
            ...
            while j < 5:
                ...
                # try:
                #     print(s_check[0])
                #     print(list(s_check[0][40:-12]).index(1) , self.interface.check_relative_state(s_check[0], self.novelty_spaces))
                #     # print(s_check[0][40:-12].index(1), self.interface.check_relative_state(s_check[0], self.novelty_spaces))
                # except:
                #     print('none')
                if self.interface.check_relative_state(s_check[0], self.novelty_spaces) or random.randint(0, 1) == 1:
            ...
            if step_idx % self.PRINT_INTERVAL == 0:
                ...
                if self.test_required:
                    avg_score, avg_winning, avg_diff, avg_score_no = self.test_v2(step_idx, seed=0, config_file=self.config_file)
